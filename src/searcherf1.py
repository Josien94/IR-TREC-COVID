...
Returns: Nothing
'''
def fieldusage1(ix, topicsDictionary, conversionDictionary, param_grid):
  if(param_grid == []):
    param_grid.append([1.0,1.0,1.0])
    param_grid = np.array(param_grid)

  stringStartTime = "\nStart with gridsearch F1 at: " + datetime.now().strftime("%d/%m/%Y %H:%M:%S") + "\n"
  write_output(stringStartTime, "outputSearcher.txt")  
  write_output("\nThe shape of param_grid F1 is {}\n".format(str(param_grid.shape)),"outputSearcher.txt")
  
  for weights in param_grid:
    write_output("Now processing weights {}\n".format(weights),"outputSearcher.txt")
    #Initiating weights
    weightTitle, weightAbstract, weightBody = weights[0], weights[1], weights[2]
    w = scoring.BM25F(B=0.75,  K1=1.2,title_B = weightTitle ,body_B= weightBody, abstract_B=weightAbstract)
    
    script_dir = os.path.dirname(__file__) #<-- absolute dir the script is in
    rel_path = "outputF1/resultsF1[{}_{}_{}].test".format(str(weightTitle), str(weightAbstract),str(weightBody))
    abs_file_path = os.path.join(script_dir, rel_path)  
    
    with open(abs_file_path,"a") as f:
      for topic_id in range(1,len(topicsDictionary)+1 ):
        #Inititate parser
        qp = MultifieldParser(["title","abstract","body"], schema=ix.schema, termclass=Variations)
...
# CHANGE DEPENDING ON WHAT FIELDUSAGE FUNCTION                                                #
    ...
    for wt in twoOptions:#np.arange(0,ev,d):
        for wa in twoOptions:#np.arange(0,ev,d):
            for wb in twoOptions:#np.arange(0,ev,d):
                param_grid.append( [wt, wa, wb])
    
    param_grid = np.array(param_grid)

    fieldusage1(ix, topicsDictionary, conversionDictionary, param_grid)
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #  
    #python3 searcher.py 
